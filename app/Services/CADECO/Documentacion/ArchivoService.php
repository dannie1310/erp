...
class ArchivoService
{
    ...
    public function cargarArchivosImagen($data){
        ...
        if(count($archivos_nombres) == 1){
            ...
            if($ok = Storage::disk('padron_contratista')->put( 'hashfiles/' .$archivo->hash_file.'.' . $nombre_explode[count($nombre_explode)-1], $decode)){
    ...
    public function cargarArchivosPDF($data){
        ...
        foreach($archivos_pdf as $key => $archivo_pdf){
            ...
            $this->validaRepetido($hash_file,$archivos_nombres[$key]->nombre);
    ...
    private function validaRepetido($hashfile, $nombre)
    {
