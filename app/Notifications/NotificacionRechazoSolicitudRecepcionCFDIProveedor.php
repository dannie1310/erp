...
class NotificacionRechazoSolicitudRecepcionCFDIProveedor extends Notification
{
    use Queueable;
    public $solicitud;
    ...
    public function toMail($notifiable)
    {
        ...
        if(file_exists($path0)){
            return (new MailMessage)
                ->subject("Rechazo de Solicitud de Recepción de CFDI")
                ->view('emails.rechazo_solicitud_recepcion_cfdi_proveedor',["solicitud"=>$this->solicitud])
                ->attach($path0);
        } else {
            return (new MailMessage)
                ->subject("Rechazo de Solicitud de Recepción de CFDI")
