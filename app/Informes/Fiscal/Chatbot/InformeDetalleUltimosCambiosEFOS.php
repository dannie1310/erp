...
ORDER BY Subquery.fecha_presunto_maxima DESC,
    ...
    public static function getEFOSPendientesCorreccion(){
...
ORDER BY sum(CASE
    ...
    public static function getUltimosEFOSCorregidos()
    {
   ...
   ListaEmpresasSAT.nombre_corto AS empresa,
    ...
    sum(CASE
        ...
        END),
          'C') AS importe_format
FROM
    SEGURIDAD_ERP.Fiscal.autocorrecciones a
INNER JOIN SEGURIDAD_ERP.Contabilidad.cfd_sat_autocorrecciones csa ON
    a.id = csa.id_autocorreccion
INNER JOIN SEGURIDAD_ERP.Contabilidad.cfd_sat ON
    csa.id_cfd_sat = cfd_sat.id
INNER JOIN SEGURIDAD_ERP.Fiscal.efos ON
    a.id_proveedor_sat = efos.id_proveedor_sat
INNER JOIN
    SEGURIDAD_ERP.Contabilidad.ListaEmpresasSAT ListaEmpresasSAT
        ON (cfd_sat.id_empresa_sat = ListaEmpresasSAT.id)

    WHERE a.estado = 6
    group by a.id, efos.rfc, efos.razon_social, ListaEmpresasSAT.nombre_corto
