...
class NoLocalizadosInforme
{
    ...
    {
        $informe["informe"][] = NoLocalizadosInforme::getPartidasProyectoEmpresa();
    ...
    {
        $informe["informe"][] = NoLocalizadosInforme::getPartidasEmpresaProyecto();
        $informe["fechas"] = NoLocalizadosInforme::getFechasInforme();
        return $informe;
    }
...
ORDER BY ListaEmpresasSAT.nombre_corto ASC;
    ...
    private static function getPartidasEmpresaProyecto(){
        $informe = DB::select("
        SELECT 'No Localizado' AS estatus,
       no_localizados.rfc,
       no_localizados.razon_social,
...
ORDER BY no_localizados.razon_social ASC;
        ...
        $informe = NoLocalizadosInforme::setSubtotalesPartidasEmpresaProyecto($informe, "NO LOCALIZADOS");
        return $informe;
    }
    ...
    private static function setSubtotalesPartidasEmpresaProyecto($partidas, $tipo){
        ...
        if(count($partidas)>0){
            ...
            foreach($partidas as $partida)
            {
                if($i_p>0){
                    if($partida["razon_social"]!=$partidas[$i_p-1]["razon_social"] ){
                        ...
                        $partidas_completas[$i]["contador"] = $contador_partidas_empresa-1;
                        $partidas_completas[$i]["acumulador"] = $acumulador_partidas_empresa;
                        $partidas_completas[$i]["etiqueta"] = "SUBTOTAL ".$partidas[$i_p-1]["razon_social"];
                        $partidas_completas[$i]["contador_cfdi"] = $contador_cfdi;
                        $partidas_completas[$i]["importe"] = $importe_cfdi;
                        $partidas_completas[$i]["importe_format"] = '$ '.number_format($importe_cfdi,2,".",",");
                        $partidas_completas[$i]["tipo"] = "subtotal";
            ...
            $partidas_completas[$i]["contador"] = $contador_partidas_empresa-1;
            $partidas_completas[$i]["acumulador"] = $acumulador_partidas_empresa;
            $partidas_completas[$i]["etiqueta"] = "SUBTOTAL ".$partidas[count($partidas)-1]["razon_social"];
            $partidas_completas[$i]["contador_cfdi"] = $contador_cfdi;
            $partidas_completas[$i]["importe"] = $importe_cfdi;
            $partidas_completas[$i]["importe_format"] = '$ '.number_format($importe_cfdi,2,".",",");
            $partidas_completas[$i]["tipo"] = "subtotal";
