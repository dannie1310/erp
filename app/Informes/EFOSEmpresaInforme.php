...
class EFOSEmpresaInforme
{
    public static function  getInforme()
    {
        $informe["informe"][] = EFOSEmpresaInforme::getPartidasInformeDefinitivos();
        $informe["informe"][] = EFOSEmpresaInforme::getPartidasInformePresuntos();
        $informe["informe"][] = EFOSEmpresaInforme::getPartidasInformeEnAclaracion();
        $informe["informe"][] = EFOSEmpresaInforme::getPartidasInformeAutocorregidos();
        $informe["informe"][] = EFOSEmpresaInforme::getPartidasInformeNoDeducidos();
        $informe["informe"][] = EFOSEmpresaInforme::getPartidasInformeDefinitivos2012();
  ...
  FROM ((((SEGURIDAD_ERP.Fiscal.efos efos
       ...
       INNER JOIN SEGURIDAD_ERP.Fiscal.ctg_efos ctg_efos
          ON (ctg_efos.rfc = efos.rfc)
 WHERE (efos.estado = 2) and cfd_sat.tipo_comprobante != 'P' and cfd_sat.cancelado != 1 and ctg_efos.estado_registro = 1
...
ORDER BY Subquery.fecha_presunto_maxima DESC,
        ...
        $informe = EFOSEmpresaInforme::setSubtotalesPartidas($informe, "PRESUNTOS");
        return $informe;
    }
  ...
  FROM ((((SEGURIDAD_ERP.Contabilidad.cfd_sat cfd_sat
       ...
       SEGURIDAD_ERP.Contabilidad.ListaEmpresasSAT ListaEmpresasSAT
          ON (cfd_sat.id_empresa_sat = ListaEmpresasSAT.id)
 WHERE (ctg_efos.estado_registro = 1) AND efos.estado = 0 AND cfd_sat.cancelado != 1 and cfd_sat.tipo_comprobante != 'P'
GROUP BY ctg_estados_efos.descripcion,
         efos.rfc,
         efos.razon_social,
         ctg_efos.fecha_definitivo,
...
ORDER BY 7 DESC
        ...
        $informe = EFOSEmpresaInforme::setSubtotalesPartidas($informe, "CORREGIDOS");
        return $informe;
    }
...
ORDER BY 7 DESC
        ...
        $informe = EFOSEmpresaInforme::setSubtotalesPartidas($informe, "EN ACLARACIÓN");
        return $informe;
    }
...
ORDER BY 7 DESC
        ...
        $informe = EFOSEmpresaInforme::setSubtotalesPartidas($informe, "NO DEDUCIDOS");
        return $informe;
    }
...
ORDER BY Subquery.fecha_devinitivo_maxima DESC,
        ...
        $informe = EFOSEmpresaInforme::setSubtotalesPartidas($informe, "DEFINITIVOS");
        return $informe;
    }
...
ORDER BY Subquery.fecha_devinitivo_maxima DESC,
        ...
        $informe = EFOSEmpresaInforme::setSubtotalesPartidas($informe, "DEFINITIVOS ANTES 2012");
        return $informe;
    }
