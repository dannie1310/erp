...
class EFOSEmpresaInformeCFDIDesglosado
{
...
ORDER BY Subquery.fecha_devinitivo_maxima DESC,
        ...
        $informe = EFOSEmpresaInformeCFDIDesglosado::setSubtotalesPartidas($informe, "DEFINITIVOS");
        $informe = EFOSEmpresaInformeCFDIDesglosado::setUUID($informe);
        return $informe;
    }
...
ORDER BY Subquery.fecha_devinitivo_maxima DESC,
    ...
    private static function setUUID($partidas){
        $i = 0;
        $partidas_completas = [];
        foreach($partidas as $partida)
        {
            ...
            if(key_exists("rfc", $partida)){
                ...
                foreach($uuid as $uuid_ind){
                    ...
                    $partidas_completas[$i]["subtotal"] = '$ '.number_format($uuid_ind["subtotal"],2,".",",");
                    $partidas_completas[$i]["total"] = '$ '.number_format($uuid_ind["total"],2,".",",");
                    $i++;
                }
            }
        }
