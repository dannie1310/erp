...
class OrdenPagoEstimacion extends Rotation
{
    ...
    function partidas()
    {
        ...
        foreach ($this->estimacion->items as $item) {
            ...
            $this->Row(['', utf8_decode($item->contrato->descripcion), '$ ' . number_format($item->importe, 2, '.', ','), utf8_decode($item->concepto && strlen($item->concepto->nivel)/4 > 1 ? $item->concepto->padre() : utf8_decode($item->contrato->descripcion)),'', '']);
        }
