...
class InformeNoLocalizados extends Rotation
{
            ...
            foreach($tipo as $partida){
                if(key_exists("tipo", $partida)){
                    ...
                    if($partida["tipo"]== "partida"){
                        $this->Row([
                            $partida["indice"],$partida["rfc"]
                            , utf8_decode($partida["razon_social"])
    ...
    private function  setEstilos($tipo){
        if($tipo == "partida"){
            ...
            $this->SetWidths([0.8,2.7,7.8,2.8,1.1,1.1,1,2.4]);
            $this->SetStyles(['DF','DF','DF','DF','DF','DF','DF','DF']);
            $this->SetRounds(['','','','','','','','']);
            $this->SetRadius([0,0,0,0,0,0,0]);
            ...
            $this->SetHeights([0.4]);
            $this->SetAligns(['C','C','L','L','L','L','R','R']);
