...
class PolizaFormatoOriginalT1 extends Rotation
{
    private $poliza;
    private $empresa;
    private $folios;
    ...
    public function partidas()
    {
        ...
        $this->Ln(0.4);
        $this->Cell(19.65, 0.5, utf8_decode($this->poliza->concepto_original), '', 0, 'C', 0);
        $this->Ln(0.48);
        $this->SetX(1);
        $cuenta_padre = '';
        ...
        foreach($this->poliza->poliza->cuentas_padres as $cuenta_padre){
            ...
            $this->Cell(5.2, 0.3, strlen($movimiento->getConceptoOriginalT1($this->poliza)) > 23 ? '  ' . utf8_decode(substr($movimiento->getConceptoOriginalT1($this->poliza), 0, 22)) . '..' : '  ' . utf8_decode($movimiento->getConceptoOriginalT1($this->poliza)), '', 1, 'L', 180);
            //strlen($movimiento->getConceptoPropuesta($this->solicitud))
            ...
            foreach ($this->poliza->poliza->getMovimientos($cuenta_padre) as $k => $movimiento)
            {
                ...
                $this->Cell(4, 0.5, strlen($movimiento->getReferenciaOriginalT1($this->poliza)) > 11 ? utf8_decode(substr($movimiento->getReferenciaOriginalT1($this->poliza), 0, 9)) . ' ..' : utf8_decode($movimiento->getReferenciaOriginalT1($this->poliza)), '', 0, 'L', 180);
