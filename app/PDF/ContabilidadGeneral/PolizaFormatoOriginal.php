...
class PolizaFormatoOriginal extends Rotation
{
    private $poliza;
    private $empresa;
    private $polizas;
    ...
    public function __construct($polizas)
    {
    ...
    function create() {
        ...
        foreach ($this->polizas as $k => $poliza)
        {
            DB::purge('cntpq');
            \Config::set('database.connections.cntpq.database', $poliza["empresa"]->AliasBDD);
            $this->poliza = $poliza["poliza"]; //Poliza::find($folio->id_poliza);
            $this->key_folio = $k;
            $this->empresa = $poliza["empresa"];
