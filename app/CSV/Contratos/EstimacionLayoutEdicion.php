...
class EstimacionLayoutEdicion implements WithHeadings, ShouldAutoSize, WithEvents
{
    protected $estimacion;
    protected $id_estimacion;
    protected $moneda;
    ...
    public function __construct($estimacion, $id_estimacion)
    {
        ...
        return [
            AfterSheet::class    => function(AfterSheet $event) {
                ...
                $event->sheet->getStyle('C4')->getNumberFormat()
                    ->setFormatCode(\PhpOffice\PhpSpreadsheet\Style\NumberFormat::FORMAT_DATE_DDMMYYYY);
                $fecha = new DateTime($this->estimacion['fecha_inicial']);
                $event->sheet->setCellValue("C4", date_format($fecha, 'd/m/Y'));
                ...
                $event->sheet->getStyle('C5')->getNumberFormat()
                    ->setFormatCode(\PhpOffice\PhpSpreadsheet\Style\NumberFormat::FORMAT_DATE_DDMMYYYY);
                $fecha = new DateTime($this->estimacion['fecha_final']);
                $event->sheet->setCellValue("C5", date_format($fecha, 'd/m/Y'));
                ...
                $datos_subcontrato = $this->estimacion['subcontrato']['partidas'];
                ...
                foreach ($datos_subcontrato as $key => $item) {
                ...
                $event->sheet->setCellValue("D" . ($i+3), $this->estimacion['observaciones']);
    ...
    {
        return array([' ','',' ',' ',' ',' ',' ',$this->estimacion['razon_social']],[],[],[],[],[],[],
