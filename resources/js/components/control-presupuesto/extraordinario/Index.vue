<template>
    <div class="row">
        <div class="col-12"  v-if="$root.can('registrar_extraordinario')" :disabled="cargando">
...
export default {
  ...
  components: {},
        data() {
            return {
                ...
                columns: [
                    ...
                    { title: 'Fecha ', field: 'fecha_solicitud', thClass: 'th_fecha', sortable: true  },
                    { title: 'Área Solicitante ', thClass:'th_c150', field: 'area_solicitante',  sortable: true  },
                    { title: 'Motivo ', field: 'motivo',  sortable: true  },
                    { title: 'Monto de la Afectación', field: 'importe_afectacion', tdClass: 'money', sortable: true },
                    { title: '% Cambio', field: 'porcentaje_cambio', tdClass: 'money', sortable: false },
        ...
        methods: {
          paginate() {
                this.cargando = true;
                return this.$store.dispatch('control-presupuesto/extraordinario/paginate', {
                    params: this.query
                })
                    .then(data => {
                        this.$store.commit('control-presupuesto/extraordinario/SET_EXTRAORDINARIOS', data.data);
                        this.$store.commit('control-presupuesto/extraordinario/SET_META', data.meta);
                    })
                    .finally(() => {
                        this.cargando = false;
                    })
            ...
            create() {
                this.$router.push({name: 'extraordinario-create'});
            },
        },
        computed: {
          solicitudes(){
                return this.$store.getters['control-presupuesto/extraordinario/extraordinarios'];
            },
            meta(){
                return this.$store.getters['control-presupuesto/extraordinario/meta'];
            },
            tbodyStyle() {
                ...
                handler(solicitudes) {
                    ...
                    solicitudes.forEach(function (solicitud, i) {
                        self.$data.data.push({
                            ...
                            importe_afectacion: solicitud.importe_afectacion_format,
