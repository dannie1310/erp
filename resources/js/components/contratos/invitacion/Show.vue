            ...
            <div class="card-body">
                ...
                <div class="row" >
                    <div class="col-md-12">
                        ...
                        <formato-invitacion-cotizacion-contrato v-bind:id="invitacion.id" v-bind:db="invitacion.base_datos" v-bind:id_obra="invitacion.id_obra" v-bind:texto="'Invitación a Cotizar'"></formato-invitacion-cotizacion-contrato>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    import FormatoInvitacionCotizacionContrato from "./FormatoInvitacionCotizacionContrato";
    export default {
        name: "InvitacionCompraShow",
        components: {
            FormatoInvitacionCotizacionContrato,
        ...
        methods: {
            find() {
                this.cargando = true;
                this.$store.commit('contratos/invitacion/SET_INVITACION', null);
                return this.$store.dispatch('contratos/invitacion/find', {
                    id: this.id,
                    params:{include: [ "carta_terminos", "formato_cotizacion"
                            ]}
                }).then(data => {
                    this.$store.commit('contratos/invitacion/SET_INVITACION', data);
                })
                .finally(()=> {
                    this.cargando = false;
                })
        ...
        computed: {
            invitacion() {
                return this.$store.getters['contratos/invitacion/currentInvitacion']
            },
        }
    }
</script>
