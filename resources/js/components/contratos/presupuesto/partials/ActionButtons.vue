<template>
    <div class="btn-group">
        ...
        <DescargaLayout v-if="$root.can('descargar_layout_presupuesto_contratista') && value.edit" v-bind:id="value.id"></DescargaLayout>
        <CargaLayout v-if="$root.can('cargar_layout_presupuesto_contratista') && value.edit" v-on:back="layout" v-bind:id="value.id"></CargaLayout>
        <router-link  :to="{ name: 'presupuesto-edit', params: {id: value.id}}" v-if="$root.can('editar_presupuesto_contratista') && value.edit" type="button" class="btn btn-sm btn-outline-info" title="Editar">
            <i class="fa fa-pencil"></i>
        </router-link>
        <PDF v-bind:id="value.id" />
        <Eliminar v-if="$root.can('eliminar_presupuesto_contratista') && value.delete" v-bind:id="value.id"></Eliminar>
