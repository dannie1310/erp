<template>
    <div class="row" v-if="solicitud_cambio">
        <div class="col-md-12">
            <span><i class="fa fa-stack-exchange"></i>Datos de Solicitud de Cambio</span>
            <table class="table table-bordered table-sm">
                <tr >
                    <th class="encabezado">
                        Folio de Solicitud
                    </th>
                    <th class="encabezado">
                        Folio de Subcontrato
                    </th>
                    <th class="encabezado">
                        Monto de Solicitud
                    </th>
                    <th class="encabezado">
                        Monto Original de Subcontrato
                    </th>
                    <th class="encabezado">
                        % Cambio Solicitud
                    </th>
                    <th class="encabezado">
                        Estado de Solicitud
                    </th>
                </tr>
                <tr>
                    <td>
                        {{ solicitud_cambio.numero_folio_format }}
                    </td>
                    <td style="text-align: center">
                        {{solicitud_cambio.subcontrato.numero_folio_format}}
                    </td>
                    <td style="text-align: right">
                        {{ solicitud_cambio.monto_format }}
                    </td>
                    <td style="text-align: right">
                        {{ solicitud_cambio.monto_original_subcontrato_format }}
                    </td>
                    <td style="text-align: right">
                        {{ solicitud_cambio.porcentaje_cambio_format }}
                    </td>
                    <td style="text-align: center">
                        <EstatusLabelGlobal v-bind:value="solicitud_cambio.estado_label"></EstatusLabelGlobal>
                    </td>
                </tr>
            </table>
        </div>
    </div>
</template>

<script>

import EstatusLabelGlobal from "../../../globals/EstatusLabel";
export default {
    name: "TablaDatosSolicitudCambioSubcontrato",
    components: {EstatusLabelGlobal},
    props: ['solicitud_cambio'],
}
</script>

<style scoped>
.encabezado{
    text-align: center; background-color: #f2f4f5
}

</style>
