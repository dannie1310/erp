...
<script>
    ...
    export default {
        ...
        methods: {
            destroy() {
                return this.$store.dispatch('finanzas/traspaso-entre-cuentas/delete', this.value.id)
                    .then(() => {
                        this.$store.commit('finanzas/traspaso-entre-cuentas/DELETE_TRASPASO', this.value.id);
                    })
            }
        }
    }
