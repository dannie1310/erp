    ...
    export default {
        ...
        data() {
            return {
                ...
                data: [],
                total: 0,
                query: { include: ['cuenta.empresa','transaccion'], sort: 'numero_folio', order: 'DESC'},
                search: '',
                cargando: false
            }
        },
        ...
        methods: {
            ...
            paginate(payload = {}) {
                this.cargando = true;
                return this.$store.dispatch('finanzas/movimiento-bancario/paginate', {params: payload})
                    .then(data => {
                        this.$store.commit('finanzas/movimiento-bancario/SET_MOVIMIENTOS', data.data);
                        this.$store.commit('finanzas/movimiento-bancario/SET_META', data.meta);
                    })
                    .finally(() => {
                        this.cargando = false;
                    })
        ...
        computed: {
            movimientos(){
                return this.$store.getters['finanzas/movimiento-bancario/movimientos'];
            },
            meta(){
                return this.$store.getters['finanzas/movimiento-bancario/meta'];
            },
        },
        watch: {
            movimientos: {
