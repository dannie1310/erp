...
<script>
    ...
    export default {
        ...
        methods: {
            destroy() {
                return this.$store.dispatch('finanzas/movimiento-bancario/delete', this.value.id)
                    .then(() => {
                        this.$store.commit('finanzas/movimiento-bancario/DELETE_MOVIMIENTO', this.value.id);
                    })
            },
            show() {
