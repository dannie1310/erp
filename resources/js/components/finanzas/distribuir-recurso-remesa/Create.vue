                                        ...
                                        <table class="table table-striped">
                                            ...
                                            <tbody>
                                                <tr v-for="(doc, i) in documentos">
                                                    ...
                                                    <td v-if = "doc.tipo_documento != 12 && doc.empresa && doc.empresa.cuentasBancariasEmpresa.data.length > 0" style="width: 15%;">
                                                        ...
                                                        >
                                                             <option value>-- Selecciona una cuenta --</option>
                                                             <option v-for="cuenta in doc.empresa.cuentasBancariasEmpresa.data" :value="cuenta.id">{{getCuentaAbono(cuenta)}}</option>
                                                        </select>
                                                        <div class="invalid-feedback"
                                                             v-show="errors.has(`id_cuenta_abono[${i}]`)">{{ errors.first(`id_cuenta_abono[${i}]`) }}
                                                        </div>
                                                    </td>
                                                    <td v-else-if="doc.tipo_documento == 12 && doc.fondo && doc.fondo.empresa && doc.fondo.empresa.cuentasBancariasEmpresa.data.length > 0 " style="width: 15%;">
                                                        <select class="form-control"
                                                                :name="`id_cuenta_abono[${i}]`"
                                                                v-model="doc.id_cuenta_abono"
                                                                v-validate="{required: doc.selected == true ? true:false}"
                                                        ...
                                                        >
                                                             <option value>-- Selecciona una cuenta --</option>
                                                             <option v-for="cuenta in doc.fondo.empresa.cuentasBancariasEmpresa.data" :value="cuenta.id">{{getCuentaAbono(cuenta)}}</option>
                                                        </select>
                                                        <div class="invalid-feedback"
                                                             v-show="errors.has(`id_cuenta_abono[${i}]`)">{{ errors.first(`id_cuenta_abono[${i}]`) }}
                                                        </div>
                                                    </td>
                                                    <td class="text-danger" style="width: 15%;" v-else-if="doc.tipo_documento != 12 && doc.empresa && doc.empresa.cuentasBancariasEmpresa.data.length == 0">Beneficiario sin cuentas bancarias registradas</td>
                                                    <td class="text-danger" style="width: 15%;" v-else-if="doc.tipo_documento == 12 && doc.fondo && doc.fondo.empresa && doc.fondo.empresa.cuentasBancariasEmpresa.data.length == 0">Beneficiario de fondo sin cuentas bancarias registradas</td>
                                                    <td class="text-danger"  style="width: 15%;" v-else>Beneficiario no registrado en cátalogo de Empresas SAO</td>

                                                    <td class="text-center" v-if="doc.empresa && doc.empresa.cuentasBancariasEmpresa.data.length > 0 && doc.tipo_cambio == 1 || doc.fondo && doc.fondo.empresa && doc.fondo.empresa.cuentasBancariasEmpresa.data.length > 0 && doc.tipo_cambio == 1 "><input type="checkbox" :value="doc.id" v-model="doc.selected"></td>
        ...
        methods: {
            ...
            getDocumentos(){
                ...
                return self.$store.dispatch('finanzas/remesa/find',{
                    id: self.id_remesa,
                    params: {
                        include: ['documentosDisponibles', 'documentosDisponibles.empresa.cuentasBancariasEmpresa.banco', 'documentosDisponibles.moneda', 'remesaLiberada', 'documentosDisponibles.fondo.empresa.cuentasBancariasEmpresa.banco']
                    }
                })
                    .then(data => {
