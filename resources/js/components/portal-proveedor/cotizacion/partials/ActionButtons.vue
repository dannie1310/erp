<template>
    <div class="btn-group">
        <router-link :to="{ name: 'cotizacion-proveedor-show', params: {id: this.value.id_invitacion}}" v-if="value.show && (value.tipo_transaccion == 17)" type="button" class="btn btn-sm btn-outline-secondary" title="Consultar co">
            <i class="fa fa-eye"></i>
        </router-link>
        <router-link :to="{ name: 'presupuesto-proveedor-show', params: {id: this.value.id_invitacion}}" v-if="value.show && (value.tipo_transaccion == 49)" type="button" class="btn btn-sm btn-outline-secondary" title="Consultar prue">
            <i class="fa fa-eye"></i>
        </router-link>
