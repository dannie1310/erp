<template>
    <div class="btn-group">
        ...
        <router-link  :to="{ name: 'invitacion-compra-show', params: {id: value.id}}" v-if="$root.can('consultar_invitacion_cotizar_compra')" type="button" class="btn btn-sm btn-outline-secondary" title="Consultar">
            <i class="fa fa-eye"></i>
        </router-link>
        <router-link  :to="{ name: 'invitacion-cotizar-compra-documentos', params: {id: value.id}}" v-if="value.show && $root.can('consultar_invitacion_cotizar_contrato')" type="button" class="btn btn-sm btn-outline-primary" title="Ver">
